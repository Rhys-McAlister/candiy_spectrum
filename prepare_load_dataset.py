...
def add_spectra_to_df(spectra_df, file_path, bins, is_mass=False):
    ...
    if spectra_df is None:
        ...
        spectra_df = pd.merge(
            spectra_df, mol_df, left_index=True, right_index=True, how="outer"
        )
...
def save_spectra_to_csv(root, files, save_path, bins, is_mass=False):
    ...
    for file_name in files:
        ...
        spectra_df = add_spectra_to_df(spectra_df, file_path, bins, is_mass)
...
def preprocess_spectra_df(spectra_df, is_mass=False, **kwargs):
    ...
    if is_mass:
        ...
        spectra_df.iloc[:, 1:] = spectra_df.iloc[:, 1:].interpolate(
            **kwargs, limit_direction="both", axis=0
        )
...
def load_dataset(data_dir, include_mass=True, **params):
    ...
    if include_mass:
        ...
        spectra_df = pd.merge(
            spectra_df, mass_df, left_index=True, right_index=True, how="inner"
        )
    ...
    total_df = pd.merge(
        spectra_df, target_df, left_index=True, right_index=True, how="inner"
    )

    return (
        total_df.values[:, :-fn_groups],
        total_df.values[:, -fn_groups:],
        list(func_grp_smarts.keys()),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", default="./data", help="Directory path containing scrapped data"
    )
    parser.add_argument(
        "--cas_list",
        default="species.txt",
    ...
    func_grp_structs = {
        func_name: Chem.MolFromSmarts(func_smarts)
    ...
    cas_df = pd.read_csv(
        args.cas_list, sep="\t", header=0, usecols=[1, 2], names=["formula", "cas"]
    )
    ...
    cas_inchi_df = pd.merge(
        cas_df, inchi_df, left_index=True, right_index=True, how="inner"
    )
    target_path = os.path.join(data_dir, "target.csv")
